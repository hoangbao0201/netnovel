...
import styles from "./ContentFormEditNovel.module.scss";

const cx = classNames.bind(styles);

export interface ContentFormEditNovelProps {
    novel?: any
}
...
const ContentFormEditNovel = ({ novel } : ContentFormEditNovelProps) => {
    const [imageThumbnail, setImageThumbnail] = useState(novel.image?.url || "/images/novel-default.png")
    const [dataImageThumbnail, setDataImageThumbnail] = useState<any>("");
    const [loadingCreateNovel, setLoadingCreateNovel] = useState<boolean>(false);
    const [loadingUploadImage, setLoadingUploadImage] = useState<boolean>(false);
    const [dataForm, setDataForm] = useState({
        title: novel.title || "",
        author: novel.author ||  "",
        description: novel.description || "",
        category: novel.category || "Tiên Hiệp",
        personality: novel.personality || "Điềm Đạm",
        scene: novel.scene || "Đông Phương Huyền Huyễn",
        classify: novel.classify || "Hệ Thống",
        viewFrame: novel.viewFrame || "Góc Nhìn Nam",
    });
    ...
    // Submit form
    const eventSubmitFormCreateNovelNew = async () => {

        // Loading
        setLoadingCreateNovel(true);

        const resCreateNovel = await axios.post("/api/novels/create", dataForm);

        // console.log(resCreateNovel.data);

        setLoadingCreateNovel(false);
    };
    ...
    const eventSubmitImage = async () => {
        ...
        const resUploadImage = await axios.post(`/api/novels/upload-thumbnail/${novel.id}`, image);
            ...
            <div className={cx("container")}>
                <div className={cx("form")}>
                    <div className={cx("form-group", "grid-form-upload-image")}>
                        ...
                        <button className={cx("button-upload-image")} onClick={eventSubmitImage}> {loadingUploadImage} Cập nhật ảnh bìa</button>
                    ...
                    <div className={cx("form-group")}>
                        ...
                        <QuillNoSSRWrapper
                            ...
                            className="customize-quill-edit-novel"
                    ...
                    <div
                        ...
                        onClick={eventSubmitFormCreateNovelNew}
                    >
                        <button className={cx("button-create-novel")}>
                            Thêm truyện {loadingCreateNovel && " - loading"}
                        </button>
                    </div>
                </div>
            </div>
...
export default ContentFormEditNovel;
